...
class TransfermarktScraper:
    ...
    def get_player_urls_from_page(self, continent_id: int, page_number: int) -> List[str]:
        """
        ...
        url = f"{base_url}/plus/0/galerie/0?ausrichtung=alle&spielerposition_id=alle&altersklasse=alle&jahrgang=0&land_id=0&yt0=Show&kontinent_id={continent_id}&page={page_number}"
    ...
    def scrape_all_players(self, max_pages: int = 20, continents: List[int] = None) -> List[Player]:
        """
        ...
        if continents is None:
            continents = range(1, 7)  # All continents
            
        all_players = []
        failed_urls = []
        
        for continent_id in continents:
            print(f"\nScraping continent {continent_id}")
            ...
            for page in range(1, max_pages + 1):
                try:
                    ...
                    player_urls = self.get_player_urls_from_page(continent_id, page)
                    
                    if not player_urls:
                        print(f"No more players found for continent {continent_id}")
                        break  # Move to next continent if no players found
                        ...
                        try:
                            ...
                            if player:
                                ...
                                if self.db.store_player(player):
                                    print(f"Successfully stored {player.name} in database")
                            ...
                            failed_urls.append(url)
                            print(f"Error scraping player: {str(e)}")
                        
                        # Delay between player requests
                        time.sleep(2)
                    ...
                    print(f"Error processing page {page} of continent {continent_id}: {str(e)}")
                    continue
